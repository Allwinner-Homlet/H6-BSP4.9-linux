...
#ifdef CONFIG_ARCH_SUN50IW1
    /*
     * it must be hear for the bit of ac_rst about ccmu set to 1.
     * and next ,we can set the reg of codec
     * at the power domain of vdd-sys.
     */
	sunxi_i2s->ac_rst = of_clk_get(node, 2);
	if (IS_ERR(sunxi_i2s->ac_rst)) {
		dev_err(&pdev->dev, "[audio-i2s]Can't get ac_rst\n");
		ret = PTR_ERR(sunxi_i2s->ac_rst);
		clk_disable(sunxi_i2s->pllclk);
		clk_disable(sunxi_i2s->moduleclk);
		goto err1;
	} else {
		clk_prepare_enable(sunxi_i2s->ac_rst);
	}
#endif
					...
					struct platform_device *pdev)
{
	...
	if (sunxi_i2s->ac_rst != NULL)
		clk_disable(sunxi_i2s->ac_rst);
#endif
	snd_soc_unregister_component(&pdev->dev);
