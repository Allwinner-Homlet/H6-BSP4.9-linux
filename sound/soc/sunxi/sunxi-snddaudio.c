...
static int sunxi_snddaudio_dev_probe(struct platform_device *pdev)
{
	...
	sunxi_snddaudio_dai_link.platform_name = "snd-soc-dummy";
	...
	if (!ret) {
		...
		if (sunxi_snddaudio_dai_link.cpu_of_node && of_property_read_u32(
			sunxi_snddaudio_dai_link.cpu_of_node, "tdm_num", &temp_val) >= 0)
