...
struct net_device *dev, char *command, int total_len)
{
	...
	ret = wldev_ioctl(dev, WLC_GET_CHANNEL, &ci, sizeof(channel_info_t), FALSE);
	if (!ret) {
...
set_channel:
	...
	if (wf_chspec_valid(chspec)) {
		...
		if (fw_chspec != INVCHANSPEC) {
			err = wldev_iovar_setint(dev, "chanspec", fw_chspec);
			if (err == BCME_BADCHAN) {
...
wl_ext_get_keep_alive(struct net_device *dev, char *command, int total_len) {
	...
	res = wldev_ioctl(dev, WLC_GET_VAR, mkeep_alive_pktp, len, FALSE);
	if (res < 0) {
...
wl_ext_set_amode(struct wl_if_info *cur_if, struct wl_apsta_params *apsta_params)
{
	...
	if (apsta_params->apstamode == IAP_MODE)
		wl_ext_ioctl(dev, WLC_DOWN, NULL, 0, 1); /* fix for 43455 */
	wl_ext_iovar_setint_bsscfg(dev, "wpa_auth", wpa_auth, cur_if->bssidx);
	if (apsta_params->apstamode == IAP_MODE)
		wl_ext_ioctl(dev, WLC_UP, NULL, 0, 1);   /* fix for 43455 */
