...
int vidioc_set_fps_ds(struct file *file, struct v4l2_fh *fh,
...
int vidioc_set_isp_debug(struct file *file, struct v4l2_fh *fh,
...
static int vin_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (inst->is_isp_used && inst->is_bayer_raw) {
		...
		case V4L2_CID_FLASH_LED_MODE:
			ret =
			    v4l2_g_ctrl(cap->pipe.sd[VIN_IND_FLASH]->ctrl_handler, &c);
			break;
		case V4L2_CID_AUTO_FOCUS_STATUS:
			ret =
				v4l2_g_ctrl(cap->pipe.sd[VIN_IND_SENSOR]->ctrl_handler, &c);
	...
	if (inst->is_isp_used && inst->is_bayer_raw) {
		...
		case V4L2_CID_TAKE_PICTURE:
			ret = v4l2_s_ctrl(NULL,
				cap->pipe.sd[VIN_IND_ISP]->ctrl_handler, &c);
			break;
		default:
			ret = -EINVAL;
			break;
		...
		case V4L2_CID_FLASH_LED_MODE:
			ret = v4l2_s_ctrl(NULL,
				cap->pipe.sd[VIN_IND_FLASH]->ctrl_handler, &c);
			break;
		case V4L2_CID_AUTO_FOCUS_START:
			sunxi_flash_check_to_start(cap->pipe.sd[VIN_IND_FLASH],
						SW_CTRL_TORCH_ON);
			ret = v4l2_s_ctrl(NULL,
				cap->pipe.sd[VIN_IND_SENSOR]->ctrl_handler, &c);
			break;
		case V4L2_CID_AUTO_FOCUS_STOP:
			sunxi_flash_stop(cap->pipe.sd[VIN_IND_FLASH]);
			ret = v4l2_s_ctrl(NULL,
