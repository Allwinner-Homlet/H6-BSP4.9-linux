...
static int read_frame(int mode)
{
	...
	if (save_flag == 0) {
		if ((count == read_num / 2) || ((count > 0) && (nplanes == 1))) {
			...
			switch (nplanes) {
			case 1:
				sprintf(fdstr, "%s/fb%d_y%d_%d_%d_%d.bin", path_name, dev_id, mode, input_size.width, input_size.height, count);
