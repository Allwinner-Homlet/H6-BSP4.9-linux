...
static int read_frame(int mode, enum v4l2_memory memory)
{
	...
	if (-1 == ioctl(fd, VIDIOC_DQBUF, &buf))
		return -1;
	printf("[csi_test]VIDIOC_DQBUF done\n");
	...
	if (-1 == ioctl(fd, VIDIOC_QBUF, &buf))
		return -1;
	printf("[csi_test]VIDIOC_QBUF done\n");
	return 0;
}
...
static int req_frame_buffers(enum v4l2_memory memory)
{
	...
	printf("[csi_test]VIDIOC_REQBUFS done\n");
	...
	for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
		...
		printf("[csi_test]VIDIOC_QUERYBUF done\n");
		switch (memory) {
...
static int camera_init(int sel, int mode)
{
	...
	printf("[csi_test]VIDIOC_S_INPUT done\n");
	...
	printf("[csi_test]VIDIOC_S_PARM done\n");
...
static int camera_fmt_set(int subch, int angle)
{
	...
	printf("[csi_test]VIDIOC_S_FMT done\n");
	...
	if (subch != 0) {
		...
		printf("[csi_test]VIDIOC_SET_SUBCHANNEL done\n");
		if (-1 == ioctl(fd, VIDIOC_SET_ROTCHANNEL, &rot)) {
			printf("VIDIOC_SET_ROTCHANNEL error!\n");
			return -1;
		}
		printf("[csi_test]VIDIOC_SET_ROTCHANNEL done\n");
	}
	...
	if (-1 == ioctl(fd, VIDIOC_G_FMT, &fmt)) {
		...
		printf("[csi_test]VIDIOC_G_FMT done\n");
