...
static int revise_current_level(void)
{
	int vol = 0;
...
#ifdef CONFIG_OF
	err = get_u32_from_fex("gpu_idle", &val);
	if (!err)
		sunxi_idle_enabled = val ? true : false;

	err = get_u32_from_fex("scene_ctrl_status", &val);
	if (!err)
		sunxi_scenectrl_enabled = val ? true : false;

	err = get_u32_from_fex("max_normal_level", &val);
	if (!err)
		sunxi_levels->max_normal_level = val;

	do {
		sprintf(second_key, "lv%d_freq", count - 1);
		err = get_u32_from_fex(second_key, &val);
		if (err)
			break;
		vf_table[count - 1].freq = val * 1000 * 1000;

		sprintf(second_key, "lv%d_volt", count - 1);
		err = get_u32_from_fex(second_key, &val);
		if (err)
			break;
		vf_table[count - 1].vol = val * 1000;
